...
public abstract class FieldDefUtil {
     ...
     */
    public static <T> FieldDef getFieldDef(final String fieldName, final Class<T> clazz)
        ...
        if (hasAlias && aliasFieldIsArray) {
            aliasFd = initFieldDef(aliasField, (FieldDef)null, getSubFieldDef(aliasField));
        } else if (hasAlias) {
    ...
    public static FieldDef getSubFieldDef(final Field field) throws SystemConfigErrorException {
    ...
    private static Field getField(final Class<?> clazz, final String fieldName)
        ...
        try {
            ...
            if (FieldUtil.isStaticOrFinal(field)) {
                throw new ErrorAnnotationConfigException("不支持final 和 static 的字段，请检查配置：" + fieldName);
            }

            return field;
        } catch (final SecurityException e) {
            throw new ErrorAnnotationConfigException("注解设置错误，没有" + fieldName + "该字段，请检查配置是否正确！");
        } catch (final NoSuchFieldException e) {
            throw new DataAccessErrorException("注解设置错误，没有" + fieldName + "该字段，请检查配置是否正确！");
        }
    }

    public static FieldDef initFieldDef(final Field field, final Class<?> clazz,
            final FieldDef aliasDef) throws SystemConfigErrorException {
        ...
        try {
            constructor = fieldDefClazz.getDeclaredConstructor(Field.class, Class.class,
                    FieldDef.class);
            constructor.setAccessible(true);
            return constructor.newInstance(field, clazz, aliasDef);
        } catch (final SecurityException e) {
            throw new SystemConfigErrorException("无法访问" + fieldDefClazz.getName() + "的构造函数", e);
        } catch (final NoSuchMethodException e) {
            throw new SystemConfigErrorException(fieldDefClazz.getName()
                    + "没有参数为Field, Class, FieldDef类型的构造函数", e);
        } catch (final IllegalArgumentException e) {
            throw new SystemConfigErrorException(fieldDefClazz.getName() + "初始化对象失败", e);
    ...
    public static FieldDef initFieldDef(final Field field, final FieldDef aliasFd,
            final FieldDef subFieldDef) throws SystemConfigErrorException {
        Class<? extends FieldDef> fieldDefClazz;
        ...
        try {
            final Constructor<? extends FieldDef> constructor = fieldDefClazz
                    .getDeclaredConstructor(Field.class, Class.class, FieldDef.class,
                            FieldDef.class);
            constructor.setAccessible(true);
            return constructor.newInstance(field, field.getType(), aliasFd, subFieldDef);
        } catch (final SecurityException e) {
            throw new SystemConfigErrorException("无法访问" + fieldDefClazz.getName() + "的构造函数", e);
        } catch (final NoSuchMethodException e) {
            throw new SystemConfigErrorException(fieldDefClazz.getName()
    ...
    public static <T> T getInstance(final Class<T> clazz) throws DataAccessErrorException {
        try {
            return clazz.newInstance();
        } catch (final InstantiationException e1) {
            throw new DataAccessErrorException(clazz.getName() + "初始化对象失败", e1);
        } catch (final IllegalAccessException e1) {
            throw new DataAccessErrorException("没有权限访问" + clazz.getName() + "对象的构造函数", e1);
        }
    }

    public static void handle(final FieldDef fieldDef, final String value, final Object target)
            throws DataNotValidException, DataAccessErrorException {
        try {
            fieldDef.handle(value, target);
        } catch (final IllegalArgumentException e) {
            throw new DataNotValidException(fieldDef.getDeclareClassName() + "对象的"
                    + fieldDef.getName() + "字段在设置" + value + "的时候发生错误", e);
        } catch (final IllegalAccessException e) {
            throw new DataAccessErrorException("没有权限访问" + fieldDef.getDeclareClassName() + "对象的"
                    + fieldDef.getName() + "字段", e);
        }
    }

    public static FieldDef getFieldDef(final String fieldName, final FieldDef aliasFieldDef,
            final Field field) throws SystemConfigErrorException {
        ...
        if (FieldDefUtil.isArray(field)) {
            ...
            return FieldDefUtil.initFieldDef(field, field.getType(), aliasFieldDef);
        }
    }
